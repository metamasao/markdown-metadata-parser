...
class MarkdownMetadata(dict):
    ...
    def __getitem__(self, key):
        ...
        return datetime.fromisoformat(item) if key == "created" else item
...
def validate_metadata_datetime(iterable):
    if not "created" in iterable.keys(): raise DatetimeDataNotFoundError("'created' is not found in metadata")
